...
(function () {
  ...
  async function sendMessageToAPI(messagesContainer, message, config, state) {
    try {
      ...
      // Mensaje de error
      clearTypingIndicator(messagesContainer);
      addBotMessage(messagesContainer, "Lo siento, ha ocurrido un error al procesar tu mensaje. Por favor, intenta nuevamente.", state);
  ...
  function addUserMessage(messagesContainer, message, state) {
  ...
  function addBotMessage(messagesContainer, message, state) {
    // Eliminar estado vacío si existe
    const emptyState = messagesContainer.querySelector('.hydrous-empty-state');
    if (emptyState) {
      messagesContainer.removeChild(emptyState);
